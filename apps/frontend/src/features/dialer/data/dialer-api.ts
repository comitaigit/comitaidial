/**
 * Thin client for the Dialer's backend calls: minting a Voice Access Token
 * for the browser softphone, listing callable prospects, and recording the
 * outcome after a call ends. Client-side — same reason as every other
 * *-api.ts in this app: these routes need the Bearer token from the
 * client-side session store.
 */

const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001/v1";

// Just enough of Person to pick who to call — kept local to this slice
// rather than importing from features/people (slices don't reach into
// each other's internals).
export type CallablePerson = {
  id: string;
  name: string;
  role: string | null;
  phone: string | null;
  account: { id: string; name: string };
};

export type CallOutcome =
  | "NO_ANSWER"
  | "VOICEMAIL"
  | "CONVERSATION_NO_PROGRESS"
  | "QUALIFIED_OBJECTION"
  | "CALLBACK_SCHEDULED"
  | "MEETING_SCHEDULED"
  | "INVALID_NUMBER"
  | "NOT_DECISION_MAKER";

export type Call = {
  id: string;
  personId: string | null;
  createdAt: string;
};

export class DialerApiError extends Error {
  constructor(
    message: string,
    public readonly status: number,
  ) {
    super(message);
    this.name = "DialerApiError";
  }
}

async function parseErrorMessage(res: Response): Promise<string> {
  try {
    const body = await res.json();
    if (typeof body.message === "string") return body.message;
    if (Array.isArray(body.message)) return body.message.join(" ");
  } catch {
    // fall through to generic message
  }
  return "Something went wrong. Please try again.";
}

async function request<T>(path: string, accessToken: string, init?: RequestInit): Promise<T> {
  const res = await fetch(`${API_BASE_URL}${path}`, {
    ...init,
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${accessToken}`,
      ...init?.headers,
    },
  });

  if (!res.ok) {
    throw new DialerApiError(await parseErrorMessage(res), res.status);
  }

  return res.json() as Promise<T>;
}

export function getVoiceToken(accessToken: string): Promise<{ token: string }> {
  return request<{ token: string }>("/calls/voice-token", accessToken, { method: "POST" });
}

export function listCallablePeople(accessToken: string): Promise<CallablePerson[]> {
  return request<CallablePerson[]>("/people", accessToken);
}

export async function getLatestCallForPerson(
  personId: string,
  accessToken: string,
): Promise<Call | null> {
  const calls = await request<Call[]>(
    `/calls?personId=${encodeURIComponent(personId)}`,
    accessToken,
  );
  return calls[0] ?? null;
}

export function updateCallOutcome(
  callId: string,
  input: { outcome: CallOutcome; durationSeconds?: number },
  accessToken: string,
): Promise<unknown> {
  return request(`/calls/${callId}`, accessToken, {
    method: "PATCH",
    body: JSON.stringify(input),
  });
}
